...
All placeholders in this template follow the
`+InvestigativeJournalism.jl+` pattern. After cloning, replace them with
...
|===
|Placeholder |Description |Example |Files
|`+InvestigativeJournalism.Jl+` |Human-readable project name
|`+My Project+` |SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md,
STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.guix,
devcontainer.json

|`+InvestigativeJournalism.jl+` |One-line description |`+A tool for X+`
|flake.guix

|`+InvestigativeJournalism.jl+` |Uppercase identifier (for Idris2
modules, C macros) |`+MY_PROJECT+` |ABI-FFI-README.md, src/abi/_.idr,
ffi/zig/_.zig

|`+{{project}}+` |Lowercase identifier (for C symbols, filenames)
|`+my_project+` |ABI-FFI-README.md, ffi/zig/*.zig

|`+InvestigativeJournalism.jl+` |Repository name (slug) |`+my-project+`
...
|`+InvestigativeJournalism.jl+` |One-line project description
